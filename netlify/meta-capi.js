...
//   META_TEST_EVENT_CODE — only used when explicitly set (e.g. while
//                          testing in Events Manager's "Test Events"
//                          tab). When this variable is absent, no
//                          test_event_code is ever attached, so
//                          production events are never silently
//                          marked as test traffic. Remove this
//                          variable (or never add it) for production.
//
// IMPORTANT: this function never logs the raw access token or raw
// (un-hashed) customer PII, and never logs Meta's full response body —
// only a small set of known-safe fields (HTTP status, fbtrace_id,
// error type/code), since Meta's error responses can in some cases
// echo back parts of the submitted payload.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // test_event_code is ONLY attached when the environment variable is
    // explicitly set. If it's absent (the normal production state),
    // nothing related to test mode is ever added to the payload — real
    // events always count as live conversions. Set META_TEST_EVENT_CODE
    // temporarily in Netlify while verifying events in Events Manager's
    // "Test Events" tab, then remove it before going live.
    ...
    if (!response.ok) {
      // Never log the full response body — Meta's error payloads can,
      // in some cases, echo back parts of the request (e.g. which
      // field failed validation), and we have no guarantee that never
      // includes anything sensitive. Only log a small, known-safe
      // subset of fields: HTTP status and Meta's own error
      // type/code/subcode, never the "message" or "error_user_msg"
      // text or any user_data/custom_data content.
      const safeError = data && data.error
        ? { type: data.error.type, code: data.error.code, error_subcode: data.error.error_subcode }
        : {};
      console.error("Meta CAPI rejected event", { eventName, status: response.status, ...safeError });
      return {
        statusCode: 502,
    ...
    // error.message from a JSON.parse or fetch failure does not contain
    // request payload content, only a description of what went wrong
    // (e.g. "Unexpected token" or "fetch failed") — safe to log as-is.
